...
1. **DA Arrears** — input: basic pay, period, old DA%, new DA% (auto-filled from `rates`). `monthly_diff = basic × (newDA−oldDA)/100`; sum over months; split cash vs GPF/CPS credit per the sanctioning GO's terms. Output month-wise table + PDF download.
