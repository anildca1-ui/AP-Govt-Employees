...
--
-- Generated from config/rates/rps-2022.json, which is the source of truth. To
-- change a rate, edit that file and regenerate — never hand-edit this SQL, or
-- the JSON and the database will disagree about what an employee is owed.
--
-- ############################################################################
-- #  EVERY VALUE IN THIS FILE IS UNVERIFIED.                                 #
-- #                                                                          #
-- #  These figures come from public web summaries of AP GOs, not from the    #
-- #  GO PDFs themselves. PLAN.md Part 5, checkpoint 3 requires each row to   #
-- #  be checked against its own GO before launch. A wrong DA percentage      #
-- #  here becomes a wrong arrears figure on someone's phone, and that is     #
-- #  the one failure this project cannot recover from.                       #
-- #                                                                          #
-- #  payload->'_unverified' is true on every seeded row. The calculators     #
-- #  read it and show a warning, so an unchecked rate cannot quietly present #
-- #  itself as authoritative.                                                #
-- ############################################################################
--
-- source_go stays null: it references documents(id), and the GOs have not been
...
-- admin has since verified are preserved.
...
insert into rates (kind, effective_from, effective_to, payload, source_go) values
  ('DA', '2018-07-01'::date, '2018-12-31'::date, '{"percent": 0.0, "applies_to": "basic_pay", "_unverified": true, "_source_go": {"go_number": null, "go_date": null, "verified": false, "note": "Base; merged into RPS-2022 fitment"}}'::jsonb, null);
insert into rates (kind, effective_from, effective_to, payload, source_go) values
...
insert into rates (kind, effective_from, effective_to, payload, source_go) values
  ('DA', '2023-01-01'::date, '2023-06-30'::date, '{"percent": 30.03, "applies_to": "basic_pay", "_unverified": true, "_source_go": {"go_number": null, "go_date": null, "verified": false, "note": "Sanctioning GO not yet identified"}}'::jsonb, null);
insert into rates (kind, effective_from, effective_to, payload, source_go) values
